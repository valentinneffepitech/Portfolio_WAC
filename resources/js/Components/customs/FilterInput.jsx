...
export const FilterInput = ({categorie, defaultChecked, manageFilter}) => {
  ...
  return (
    <div className='my-1 md:w-1/6 lg:w-[10%] mx-2'>
        ...
        <label htmlFor={`category_${categorie.id}`} className={`py-6 px-4 w-full flex cursor-pointer transition-all border-2 border-transparent hover:border-white duration-300 ${defaultChecked.includes(categorie.id) ? "bg-white text-[#1d1d1d]": "border-transparent"}`}>
            {categorie.name}
        </label>
    </div>
  )
